...
static const GUID CLSID_ChainHost = {0x2a2a2a2a,0x2a2a,0x2a2a,{0x2a,0x2a,0x2a,0x2a,0x2a,0x2a,0x2a,0x2a}};

class ChainProfiler : public ICorProfilerCallback3 {
...
class ChainFactory : public IClassFactory {
    std::atomic<ULONG> _ref{1};
public:
    HRESULT STDMETHODCALLTYPE QueryInterface(REFIID riid, void** ppv) override {
        if (!ppv) return E_POINTER;
    ...
    HRESULT STDMETHODCALLTYPE CreateInstance(IUnknown* pUnkOuter, REFIID riid, void** ppv) override {
        ...
        ChainProfiler* p = new ChainProfiler();
...
extern "C" HRESULT STDAPICALLTYPE DllGetClassObject(REFCLSID rclsid, REFIID riid, void** ppv) {
    if (rclsid != CLSID_ChainHost) return CLASS_E_CLASSNOTAVAILABLE;
    ChainFactory* f = new ChainFactory();
...
static std::atomic<unsigned long long> g_next{1};
static std::atomic<unsigned long long> g_session{0};

extern "C" HRESULT STDAPICALLTYPE ProfilerChain_AttachOffer(const GUID* attemptedClsid,
    const wchar_t* attemptedPath, const BYTE* clientData, UINT cbClientData, unsigned long long* outSessionId)
{
    (void)attemptedClsid; (void)attemptedPath; (void)clientData; (void)cbClientData;
    if (!g_active.load()) {
        g_active.store(true);
        unsigned long long sid = g_next.fetch_add(1);
        g_session.store(sid);
        if (outSessionId) *outSessionId = sid;
        std::fprintf(stderr, "[ChainHost] Accept attach sid=%llu\n", sid);
        return S_OK;
    }
    std::fprintf(stderr, "[ChainHost] Decline attach; already active sid=%llu\n", (unsigned long long)g_session.load());
    return S_FALSE;
}

extern "C" void STDAPICALLTYPE ProfilerChain_VirtualDetach(unsigned long long sessionId, unsigned int reason)
{
    std::fprintf(stderr, "[ChainHost] VirtualDetach sid=%llu reason=%u\n", sessionId, reason);
    if (g_active.load() && g_session.load()==sessionId) {
